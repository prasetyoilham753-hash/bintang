...
**OPEN DECISIONS:** Admin Authorization Strategy.

## 35. PHASE 1 — BUILD FOUNDATION LOG
**Date:** Recorded at implementation time.
**Change:** Added the missing `firebase` package (`^12.18.0`) to `package.json` dependencies — the codebase already imported `firebase/app`, `firebase/auth`, and `firebase/firestore` but the package itself was never declared, so installs/builds could not resolve those modules.
**Also changed:** Enabled `resolveJsonModule` in `tsconfig.json` so the existing default import of `firebase-applet-config.json` in `src/services/firebase/config.ts` type-checks now that Firebase's types are present.
**Not changed:** Firebase initialization pattern (`getApps`/`getApp`/`initializeApp` singleton), Firebase Project ID (`bintangprasetyo-porto`), authentication method, Firestore rules, routing, UI, or any content.
**Firebase Project ID confirmed unchanged:** `bintangprasetyo-porto`.
**Lockfile:** Not regenerated by this change — see build notes; run `bun install` once to update `bun.lock`.
